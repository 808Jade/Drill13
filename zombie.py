...
class Zombie:
    ...
    def ball_less_than(self, ball_count_1, ball_count_2):
        return ball_count_1 - ball_count_2

    def ball_more_than(self, ball_count_1, ball_count_2):
        return ball_count_1 - ball_count_2
    ...
    def move_slightly_to_2(self, tx, ty):
    ...
    def is_boy_have_low(self):
        if (self.ball_count > play_mode.boy.ball_count):
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL

    # def is_boy_have_high(self):
    #     if self.ball_less_than(play_mode.boy.ball_count, self.ball_count):
    #         return BehaviorTree.FAIL
    #     else:
    #         return BehaviorTree.SUCCESS

    def is_boy_have_high(self):
        if (self.ball_more_than(play_mode.boy.ball_count, self.ball_count)):
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL
    ...
    def run_from_boy(self, r=0.5):
        self.state = 'Walk'
        self.move_slightly_to_2(play_mode.boy.x, play_mode.boy.y)
        if self.distance_less_than(play_mode.boy.x, play_mode.boy.y, self.x, self.y, r):
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.RUNNING
    ...
    def build_behavior_tree(self):
        ...
        c2 = Condition('소년이 가진 공이 좀비가 가진 공보다 적은가?', self.is_boy_have_low)
        ...
        c3 = Condition('소년이 가진 공이 좀비가 가진 공보다 큰가?', self.is_boy_have_high)
        a5 = Action('소년으로부터 도망', self.run_from_boy)
        SEQ_run = Sequence('도망',    c1, c3, a5)

        root = SEL_chase_or_wander_or_run = Selector('추적 또는 배회 또는 도망',  SEQ_chase_boy, SEQ_run, SEQ_wander )
